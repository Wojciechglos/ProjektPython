...
from ui import Aplikacja_do_sprawdzania_jakosci_powietrza, Menu, MapaStacji, StronaWyboruStacji, WyborSensora, DataAnalysisPage
...
def test_update_stations(mock_api_stations, app):
    mock_api_stations.return_value = [
        {'stationName': 'Station1', 'id': 1},
        {'stationName': 'Station2', 'id': 2}
    ]
...
def test_show_sensors(mock_api_sensors, app):
    mock_api_sensors.return_value = [
        {'param': {'paramName': 'Sensor1'}, 'id': 1},
        {'param': {'paramName': 'Sensor2'}, 'id': 2}
    ]
...
def test_analyze_historical_data(mock_calculate_trends, mock_analyze_data, mock_fetch_historical_data, app):
    ...
    frame = app.frames["DataAnalysisPage"]
